...
import daris.lifepool.client.CiteableIdUtils;
...
public class Upload extends Task<Void> {
    ...
    public static final String APP = "lifepool-data-upload";

    public static final String PROPERTIES_FILE = System.getProperty("user.home") + "/.daris/" + Upload.APP
            + ".properties";

    public static final int INDENT = 4;

    static UploadSettings loadSettingsFromPropertiesFile() {
        return UploadSettings.loadFromPropertiesFile(PROPERTIES_FILE);
    }
    ...
    public Upload(UploadSettings settings, Logger logger) throws Throwable {
        _settings = settings;
        _settings.setApp(APP);
        setLogger(logger);
    }

    public Upload(UploadSettings settings) throws Throwable {
